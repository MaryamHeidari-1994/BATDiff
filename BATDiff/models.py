...
def _to_tensor(x_np, device, dtype=torch.float32):
    return torch.from_numpy(x_np).to(device=device).type(dtype)


def atrous_decompose_batch(x, level=1, wavelet='b3', scale_factor=2):
    b, c, h, w = x.shape
    device = x.device
...
def atrous_reconstruct_batch(coarse, target_size, level=1, wavelet='b3', scale_factor=2):
    b, c, h_coarse, w_coarse = coarse.shape
    H_target, W_target = target_size
    device = coarse.device
    res = np.zeros((b, c, H_target, W_target), dtype=np.float32)
    ...
    for bi in range(b):
        for ch in range(c):
            ...
            if base.shape != (H_target, W_target):
                base = np.array(_nearest_upsample_2d(base, (H_target, W_target)), dtype=np.float32)
            res[bi, ch] = base
...
def ImageUpsampleNearest(img2d, target_shape):
    ...
    factor_h = max(1, Ht // Hs)
    factor_w = max(1, Wt // Ws)
    return np.repeat(np.repeat(img2d, factor_h, axis=0), factor_w, axis=1)
...
class BATDiffNet(nn.Module):
    def __init__(
        ...
        self.l1 = BATDiffConvBlock(channels, half_dim, time_emb_dim=time_dim)
    ...
    def forward(self, x, time, scale, parent=None):
...
class MultiScaleGaussianDiffusion(nn.Module):
    def __init__(
            ...
            *,
            ...
            omega=0.5,
            # new args for atrous wavelet ops
            use_atrous=False,
            atrous_level=1,
            atrous_wavelet='b3',
    ):
        ...
        # wavelet / atrous settings
        self.use_atrous = use_atrous and _HAS_PYWT
        self.atrous_level = atrous_level
    ...
    # ... بقیه متدها بدون تغییر اساسی (به جز sample_via_scale که آپسمپل جایگزین می‌شود)
    # برای خوانایی آن‌ها را حفظ می‌کنیم و فقط تغییرات لازم را اعمال می‌کنیم.
    ...
    def sample_via_scale(self, batch_size, img, s, scale_mul=(1, 1), custom_sample=False, custom_img_size_idx=0, custom_t=None, custom_image_size=None):
        ...
        if self.use_atrous:
            ...
            try:
                img = atrous_reconstruct_batch(img, target_size=image_size, level=self.atrous_level, wavelet=self.atrous_wavelet, scale_factor=self.scale_factor)
            except Exception as e:
                
                print("atrous reconstruction failed:", e)
