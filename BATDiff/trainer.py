...
from BATDiff.models import EMA
...
class MultiscaleTrainer(object):
    ...
    def __init__(
        ...
        for i in range(n_scales):
            ...
            self.dl_list.append(
                cycle(data.DataLoader(self.ds_list[i], batch_size=ts, shuffle=True, pin_memory=True)))

            if i > 0:
                Data = next(self.dl_list[i])
                self.data_list.append((Data[0].to(self.device), Data[1].to(self.device)))
            else:
                Data0 = next(self.dl_list[i]).to(self.device)
                self.data_list.append((Data0, Data0))
    ...
    def train(self):
        ...
        while self.step < self.train_num_steps:
            ...
            for i in range(self.gradient_accumulate_every):
                ...
                loss_avg += loss.item()
                backwards(loss / self.gradient_accumulate_every, self.opt, model=self.model)
    ...
    def sample_scales(self, scale_mul=None, batch_size=16, custom_sample=False, custom_image_size_idxs=None,
        ...
        for i in range(n_scales):
            if start_noise and i == 0:
                ...
                prev_img = samples_from_scales[i - 1]
                cur_init = prev_img.clone()
            ...
            safe_desc = str(desc).replace(":", "-").replace(" ", "_").replace(".", "-")
            short_name = str(res_sub_folder)[:20]
            file_path = final_results_folder / f"{short_name}_s{i}_{safe_desc}.png"
            utils.save_image(final_img, str(file_path), nrow=4)

        if save_unbatched:
            final_results_folder = Path(str(self.results_folder / f'final_samples_unbatched_{desc}'))
            final_results_folder.mkdir(parents=True, exist_ok=True)
            for b in range(batch_size):
              safe_folder = str(res_sub_folder)[:30]  # حداکثر 30 کاراکتر
              file_path = final_results_folder / f"{safe_folder}_b{b}.png"
              utils.save_image(final_img[b], str(file_path))
